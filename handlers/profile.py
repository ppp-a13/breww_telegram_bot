...
async def user_profile_information(
    ...
    await message.answer(
        ...
        f'Ваш баланс - {user.view_balance} монеток',
...
async def user_deposit_action_cancel(callback_query: types.CallbackQuery, state: FSMContext, user_repository: UserRepository):
...
async def user_deposit_amount(message: types.Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer('Ведите целое число')
        return
    ...
    await message.answer(
        f'Подтверждаете пополнение баланса на {amount} монеток?',
...
async def apply_user_deposit(
    ...
    await callback_query.message.edit_text(
        f'Баланс успешно пополнен на {deposit_amount} монеток!',
        reply_markup=profile_keyboard.break_action_and_back_to_main_menu(
            'Профиль'
        )
    )
...
async def buy_tea_action(callback_query: types.CallbackQuery, callback_data: BuyTeaCallBackData, tea_repository: TeaRepository):
    await callback_query.answer()
